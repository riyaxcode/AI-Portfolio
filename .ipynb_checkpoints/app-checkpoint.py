...
# 1. Page Configuration (Humesha sabse upar hona chahiye)
...
# ==========================================
    ...
    st.subheader(f"Data from 2010 to 2023 for {user_input}")
    ...
    try:
        ...
        train_len = int(len(close_data) * 0.70)
        data_training_array = close_data[0:train_len]
        data_testing_array = close_data[train_len:]

        scaler = MinMaxScaler(feature_range=(0, 1))
        scaler.fit(data_training_array)

        past_days = data_training_array[-time_step:]
        final_array = np.vstack((past_days, data_testing_array))

        input_data = scaler.transform(final_array)
        ...
        st.error("⚠️ Error: 'keras_model.h5' not found. Ensure the file is in the same folder.")
    except Exception as e:
...
# ==========================================
    ...
    with col1:
        ...
        HouseAge = st.number_input("House Age (Years)", min_value=1.0, max_value=100.0, value=28.6)
        AveRooms = st.number_input("Average Rooms", min_value=1.0, max_value=20.0, value=5.4)
        AveBedrms = st.number_input("Average Bedrooms", min_value=0.5, max_value=10.0, value=1.09)
